...
M.opts = function()
	...
	N.dashboard = {
		preset = {
			...
			keys = {
				{
					key = "e",
					icon = icons.documents.folder.tree,
					desc = "Open explorer",
					action = function()
				...
				{
					key = "f",
					icon = icons.documents.file.find,
					desc = "Find files",
					action = function()
				...
				{
					key = "w",
					icon = icons.documents.text.find,
					desc = "Find text",
					action = function()
				...
				{
					key = "r",
					icon = icons.documents.file.recent,
					desc = "Find recents",
					action = function()
				...
				-- { key = "S", icon = icons.others.find.session, desc = "Find sessions", action = "<cmd>Telescope session-lens<cr>", },
				...
				-- { key = "S", icon = icons.others.find.session, desc = "Find sessions", action = function() require("persistence").select() end },

				{
					key = "s",
					icon = icons.others.restore,
				...
				{
					key = "S",
					icon = icons.others.find.session,
					desc = "Find sessions",
					action = function()
			...
			icon = function(item)
				if item.file then
					if item.icon == "file" then
						return { icons.documents.file.default, width = 2, hl = "icon" }
					elseif item.icon == "directory" then
						return { icons.documents.folder.closed, width = 2, hl = "icon" }
					end
				end
		...
		sections = {
			...
			{
				icon = icons.documents.file.default,
				title = "Recent Files",
			...
			{
				icon = icons.documents.folder.closed,
				title = "Projects",
				section = "projects",
				indent = 2,
				padding = { 2, 0 },
	...
	N.indent = {
		...
		chunk = {
			enabled = true,
			char = {
				corner_top = icons.ui.border.top.left,
				corner_bottom = icons.ui.border.bottom.left,
			},
		},
	}
			...
			javascript = {
				keys = {
					["source"] = {
						...
						function(self)
							local name = "scratch."
								.. vim.fn.fnamemodify(vim.api.nvim_buf_get_name(self.buf), ":w !node")
...
M.config = function(_, opts)
	...
	local toggle_leader = keys.leader.toggle
	local terminal_leader = keys.leader.terminal
	local scratch_leader = keys.leader.scratch
	local option_leader = keys.leader.option
	...
	toggle
		...
		:map(option_leader .. "cx") -- stay cursor at middle row
	toggle
		.option(
			"sidescrolloff",
			{ off = vim.o.sidescrolloff > 0 and vim.o.sidescrolloff or 0, on = 999, name = "Middle column" }
		)
		:map(option_leader .. "cy") -- stay cursor at middle column
	...
	map(0, { "n" }, scratch_leader, "", { desc = "Scratch" })
	map(0, { "n" }, scratch_leader .. "ss", function()
		scratch()
	end, { desc = "Press 0-9 & Toggle" }) -- scratch
	map(0, { "n" }, scratch_leader .. "sl", function()
		scratch.list()
	end, { desc = "List" }) -- scratch list
	map(0, { "n" }, scratch_leader .. "se", function()
		scratch.select()
	end, { desc = "Select" }) -- scratch select

	-- snacks terminal
	map(0, { "n" }, terminal_leader, "", { desc = "Terminal" })
	map(0, { "n" }, terminal_leader .. "t", function()
		terminal()
	end, { desc = "Press 0-9 & Toggle" }) -- terminal toggle
	map(0, { "n" }, terminal_leader .. "h", function()
		terminal(nil, { win = { position = "left" } })
	end, { desc = "Press 0-9 & Toggle left" }) -- terminal left
	map(0, { "n" }, terminal_leader .. "j", function()
		terminal(nil, { win = { position = "bottom" } })
	end, { desc = "Press 0-9 & Toggle down" }) -- terminal bottom
	map(0, { "n" }, terminal_leader .. "k", function()
		terminal(nil, { win = { position = "top" } })
	end, { desc = "Press 0-9 & Toggle up" }) -- terminal top
	map(0, { "n" }, terminal_leader .. "l", function()
		terminal(nil, { win = { position = "right" } })
	end, { desc = "Press 0-9 & Toggle right" }) -- terminal right
	map(0, { "n" }, terminal_leader .. "f", function()
		terminal(
			nil,
			{ win = { position = "float", enter = true, title = " Terminal " .. tostring(vim.v.count1) .. " " } }
		)
		vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes("<esc>", true, false, true), "n", false)
	end, { desc = "Press 0-9 & Toggle float" }) -- terminal float

	-- snacks notifier
	map(0, { "n" }, "<esc>", "<esc><cmd>lua Snacks.notifier.hide()<cr>", { desc = "Dismiss all" }) -- dismiss all notifier
	map(0, { "n" }, toggle_leader .. "N", function()
		notifier.show_history()
	end, { desc = "Notification history" }) -- notification history
	...
	if vim.fn.exists(":FzfLua") == 0 and vim.fn.exists(":Telescope") == 0 then
		...
		map(0, { "n" }, finder_leader .. "b", function()
			picker.buffers()
		end, { desc = "Buffers" }) -- buffers
		-- map(0, { "n" }, finder_leader .. "c", "<cmd>FzfLua blines<cr>", { desc = "Current" })
		map(0, { "n" }, finder_leader .. "C", function()
			picker.commands()
		end, { desc = "Commands" }) -- commands
		map(0, { "n" }, finder_leader .. "f", function()
			picker.files()
		end, { desc = "Files" }) -- files
		map(0, { "n" }, finder_leader .. "F", function()
			picker.resume()
		end, { desc = "Resume" }) -- resume
		map(0, { "n" }, finder_leader .. "h", function()
			picker.help()
		end, { desc = "Helps" }) -- help tags
		map(0, { "n" }, finder_leader .. "m", function()
			picker.marks()
		end, { desc = "Marks" }) -- marks
		-- map(0, { "n" }, finder_leader .. "o", function() end, { desc = "Old files" }) -- old files/recent files
		map(0, { "n" }, finder_leader .. "r", function()
			picker.registers()
		end, { desc = "Registers" }) -- registers
		map(0, { "n" }, finder_leader .. "u", function()
			picker.undo()
		end, { desc = "Undo" }) -- undo
		map(0, { "n" }, finder_leader .. "w", function()
			picker.grep()
		end, { desc = "Words" }) -- words/chars
		map(0, { "n" }, finder_leader .. "l", function()
			picker.loclist()
		end, { desc = "Loclist" }) -- loclist
		map(0, { "n" }, finder_leader .. "q", function()
			picker.qflist()
		end, { desc = "Quickfix" }) -- qflist
		map(0, { "n" }, finder_leader .. "z", function()
			picker.colorschemes()
		end, { desc = "Colorscheme" }) -- colorschemes
	end

	-- if neo-tree isn't active then these will works
	if vim.fn.exists(":Neotree") == 0 then
		map(0, { "n" }, explorer_leader, "", { desc = "Explorer" })
		map(0, { "n" }, explorer_leader .. "e", function()
			explorer({ sources = { explorer = { hidden = false } } })
		end, { desc = "Toggle" }) -- explorer without hidden files
		map(0, { "n" }, explorer_leader .. "E", function()
			explorer({ sources = { explorer = { hidden = true } } })
		end, { desc = "Toggle (Hidden)" }) -- explorer with hidden files
	end
end
